...
#  Modified by Zheng by adding the "sorted" on 01/03/2025
        files=sorted(glob.glob(thisDateDir)) 
        for fname in files:            
            print ("Checking " + fname)
            if os.path.isfile(fname):
                print ("reading " + fname)
		
                # On 01/03/2025 Zheng added "try-except-else-finally" to prevent the python
                # running from failure when the NC file is corrupted or unknown file format.
                try:
                    nc  = n4.Dataset(fname)
